...
app.get('/api/satellites', async (req, res) => {
  try {
    ...
    try {
      ...
      const nearest50 = satellitesWithPositions
        .sort((a, b) => a.distance - b.distance)
        .slice(0, 50);

      console.log(`Selected 50 nearest satellites (closest: ${(nearest50[0]?.distance / 1000).toFixed(2)} km)`);

      // Save the nearest 50 TLE pairs
      const tlePairsToSave = nearest50.map(sat => ({
        name: sat.name,
        tle1: sat.tle1,
        tle2: sat.tle2
      }));
      ...
      const positionsToSave = nearest50.map((sat, idx) => ({
        index: idx + 1,
        name: sat.name,
        tle1: sat.tle1,
        tle2: sat.tle2,
